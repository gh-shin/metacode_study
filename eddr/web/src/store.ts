...
// 모드 전환 규칙 (M3·M4 확장) —
// · browse → dateDetail(점·썸네일 마커·클러스터 시트의 날짜 헤더 탭): 닫기 → browse.
...
export type Mode = 'browse' | 'search' | 'dateDetail' | 'clusterDetail' | 'geocode'
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  closeSearch: () => set({ mode: 'browse', searchResult: null, returnTo: null }),
  ...
  flyTo: (center, zoom) =>
    set({
      cameraRequest: { type: 'flyTo', center, zoom, seq: nextSeq(get) },
    }),
