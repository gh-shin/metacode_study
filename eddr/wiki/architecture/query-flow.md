...
QueryExtractor (src/eddr/query/extract.py — gemma4:e2b, ollama structured output, temp 0)
  ...
  ├─ 출력: {keywords_en[], date_from, date_to, countries[], cities[], answer_type}  ※ 지명은 한국어, answer_type=fact|photo_list
  └─ 폴백: JSON 실패 → 1회 재시도 → 임베딩-only(fallback=true)
        │
        ├─ 지역명 → trips 조회 → trip_ids (GPS 무 사진 우회 — 구 Claude의 list_trips 역할)
        ├─ fact/date intent → trip_summary 구성 (is_date_intent fallback으로 짧은 "언제 갔어" 오분류 보정)
        ▼
QueryService.semantic_search_photos (src/eddr/query/tools.py — 내부 검색 서비스)
  ...
  ├─ production 기본: k=20, keywords_en 미전달(RRF OFF) — precision-best 조건
  ├─ lexical leg: FTS5 BM25 (실험·fallback용, broad keyword는 0.25 가중)
...
서버 그룹핑: KST 달력일별, 그룹 정렬 = 그룹 내 최고 rank(관련도순, D26-⑦)
        ▼
{interpretation(해석 칩·answer_type·fallback), groups[+좌표], trip_summary[]} — 채팅형 텍스트 답변 없음. 지도 하이라이트 + flyTo
```
...
| 질의 유형 | 주 신호 | 비고 |
|---|---|---|
| *무엇* (이벤트·객체·음식) | 임베딩 leg 기본 | 캡션검색 recall@10 0.70 (D19 PASS). keywords_en BM25는 기본값이 아니라 실험·fallback용 |
| *어디서* (고유지명) | countries/cities 필터 **OR trip_ids** | 영어 캡션의 한글 지명 약점 → 메타 필터. GPS 무 사진은 trip_ids가 회수. M4 수동 지오코딩·M5 메모가 커버리지 확대 |
| *언제* ("작년 여름", "이탈리아 언제 갔어") | 추출된 date_from/to + fact형 trip_summary | gemma 상대날짜 해석 + deterministic date intent fallback. "하루"=KST 달력일(ADR-0009 §6) |
