...
> **현재 코드 상태(2026-06-11)**: D25 M1(API 서버)·M2(채팅 SPA)까지 구현된 상태에서 D26 전환 확정 — M0(문서) 완료. 채팅 표면(`routes/chat.py`·`ChatPane`·Gradio)은 **M3에서 일괄 삭제** 예정이며 그때까지 코드에 잔존. 구현 이력: [[web-server-m1]](../impl-log/web-server-m1.md)·[[web-spa-m2]](../impl-log/web-spa-m2.md)(채팅 SPA — D26으로 superseded).
...
routes/search → QueryExtractor(gemma4:e2b) → QueryService(RRF) → KST 날짜 그룹·관련도순
   ▼
{interpretation, groups[{date, place, photos[+좌표]}]}  → lane 렌더 + 지도 하이라이트
