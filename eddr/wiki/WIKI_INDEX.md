...
- [query-flow.md](architecture/query-flow.md) — D26 로컬 검색 파이프라인: gemma4:e2b 추출(answer_type)→RRF 융합(broad lexical 0.25)→날짜 lane+fact trip summary (PLAN §6 v2)
- [web-app.md](architecture/web-app.md) — D26 웹 앱: FastAPI+React 지도 검색 SPA — `/api/search` lane+trip_summary 응답·ResultsSheet fact 카드
...
- [retrieval-quality.md](impl-log/retrieval-quality.md) — 검색품질(2026-06-11, 2026-06-22 follow-up): 전체 RRF·reranker 실측 기각, 채택은 adaptive over-fetch·질의 instruction·필터 의미론 가이드·broad lexical 하향(0.25) — 벤치 norm 0.378→0.739, golden PASS 10 / 보류 1 유지.
...
- [local-search-m3.md](impl-log/local-search-m3.md) — D26 M3(2026-06-12, 2026-06-21 follow-up): 로컬 검색 전환(추출 게이트·OR 스코프·KST lane)+intent router answer_type·fact trip_summary+채팅 일괄 삭제(외부 LLM 0회)+golden v2 러너
