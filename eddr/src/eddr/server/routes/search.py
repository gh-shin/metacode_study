...
# precision-best 기본 검색 폭 — full matrix 기준 instruction ON · k=20 · RRF OFF.
_SEARCH_K = 20
...
async def search(
    ...
    (원문 질의 vector 검색) → KST 달력일 그룹핑. 그룹 정렬은 그룹 내
    ...
    date_intent = is_date_intent(query)
    if date_intent and extracted.answer_type != "fact":
        extracted = replace(extracted, answer_type="fact")
    trips = []
    if extracted.answer_type == "fact" and extracted.countries:
        trips = await run_in_threadpool(
            state.service.list_trips,
            countries=list(extracted.countries),
            date_from=extracted.date_from,
            date_to=extracted.date_to,
        )
    # ponytail: 장소 없는 fact 질의는 최근 trip 전체를 뿌리지 않는다. 필요해지면 별도 intent 추가.
    lanes = group_by_kst_date(results, order_by_date=date_intent or extracted.answer_type == "fact")
    return {
        ...
        "trip_summary": [asdict(trip) for trip in trips],
        "total": len(results),
    }
