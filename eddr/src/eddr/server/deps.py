...
from eddr.query.retrieval_config import RetrievalConfig, get_retrieval_config
...
class ServerConfig:
    ...
    Attributes:
        ...
        ollama_host: 질의 추출기(QueryExtractor)의 Ollama 서버 URL.
            None이면 기본 로컬 호스트.
        retrieval_config: 검색 실험 변형. None이면 EDDR_VARIANT env, 없으면 baseline.
    """
    ...
    def resolved_retrieval_config(self) -> RetrievalConfig:
        """명시 설정 또는 EDDR_VARIANT env를 baseline 기본값으로 해석한다."""
        if self.retrieval_config is not None:
            return self.retrieval_config
        return get_retrieval_config(os.environ.get("EDDR_VARIANT"))
...
class AppState:
    ...
    Attributes:
        ...
        retrieval_config: 검색 실험 변형. baseline이면 검색 코어는 기존 경로 그대로다.
...
def build_state(config: ServerConfig) -> AppState:
    ...
    db.initialize()
    retrieval_config = config.resolved_retrieval_config()
    reranker = build_reranker(retrieval_config.rerank)
    query_expander = build_query_expander(
        retrieval_config.expansion, ollama_host=config.ollama_host
    )
