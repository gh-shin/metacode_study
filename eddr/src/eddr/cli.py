...
def _build_parser() -> argparse.ArgumentParser:
    ...
    db_normalize.add_argument(
        "--backup",
        type=Path,
        default=Path("data/eddr_backup_pre_kst_20260611.sqlite"),
        help="백필 전 DB 백업 경로 (이미 있으면 재복사 안 함)",
    )
...
def _cmd_golden(args: argparse.Namespace) -> int:
    ...
    from eddr.query.golden import load_golden_set, run_golden_set, write_report
    ...
    try:
        rows = run_golden_set(
            questions,
            lambda query: run_search(extractor, service, query),
