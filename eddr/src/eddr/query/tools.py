...
_LEXICAL_POOL = 1000
# ponytail: 고정 broad 목록. noisy term이 더 쌓이면 DF 기반 정책으로 교체.
_BROAD_LEXICAL_KEYWORDS = frozenset({"food", "temple", "flower", "flowers", "travel"})
_BROAD_LEXICAL_WEIGHT = 0.25
...
class NotesBM25Store(Protocol):
    """한국어 메모 텍스트 BM25 검색 프로토콜."""
    ...
    def search(self, query: str, k: int) -> list[str]:
        """질의와 가까운 메모의 photo_id를 순위대로 돌려준다."""
        ...
...
class QueryExpander(Protocol):
    """질의를 여러 검색 질의로 확장하는 실험용 프로토콜."""

    def expand(self, query: str) -> list[str]:
        """원질의를 포함한 검색 질의 목록을 돌려준다."""
        ...
...
class QueryService:
    ...
    def __init__(
        ...
        notes_bm25: NotesBM25Store | None = None,
    ):
        """검색 서비스를 조립한다.
        ...
        Args:
            ...
            query_expander: 질의를 여러 embedding 질의로 확장하는 실험 주입점.
                None이면 원질의 1개만 사용해 기존 경로와 같다.
            note_store: 메모 임베딩 컬렉션(eddr_note_text_v1) 핸들. None이거나 비어
                있으면 note leg를 생략한다 — 기존 호출처 호환 (D26 M5).
            notes_bm25: SQLite notes 텍스트 BM25 leg. None이거나 비어 있으면 생략.
        """
        self.db = db
    ...
    def semantic_search_photos(
        ...
        k = _clamp(k)
        queries = self.query_expander.expand(query) if self.query_expander is not None else [query]
        queries = [item for item in queries if item.strip()] or [query]
        embed_texts = [
            self.query_embed_template.format(query=item) if self.query_embed_template else item
            for item in queries
        ]
        embeddings = self.embedding_client.embed_texts(embed_texts)
        filters = PhotoQueryFilters(
            date_from=_kst_bound(date_from),
            date_to=_kst_bound(date_to, end=True),
            countries=tuple(countries or ()),
        ...
        broad_lexical_ids: list[str] = []
        if keywords:
            precise_terms: list[str] = []
            broad_terms: list[str] = []
            for term in (kw.replace('"', " ").strip() for kw in keywords):
                if not term:
                    continue
                if term.lower() in _BROAD_LEXICAL_KEYWORDS:
                    broad_terms.append(term)
                else:
                    precise_terms.append(term)
            precise_match = " OR ".join(f'"{term}"' for term in precise_terms)
            broad_match = " OR ".join(f'"{term}"' for term in broad_terms)
            if precise_match:
                raw_lexical = self.db.search_caption_photo_ids(precise_match, limit=_LEXICAL_POOL)
            ...
            if broad_match:
                raw_broad = self.db.search_caption_photo_ids(broad_match, limit=_LEXICAL_POOL)
                broad_lexical_ids = self.db.filter_photo_ids(raw_broad, filters)
        # note leg(D26 M5) — 빈 컬렉션이면 조회 자체를 생략한다(오버헤드 0).
        note_count = self.note_store.count() if self.note_store is not None else 0
        notes_bm25_count = self.notes_bm25.count() if self.notes_bm25 is not None else 0
        ...
        while True:
            ...
            for embedding in embeddings:
                ...
                store_exhausted = store_exhausted and len(hits) < pool_k
                ids: list[str] = []
                for hit in hits:
                    if hit.photo_id and hit.photo_id not in ids:
                        ids.append(hit.photo_id)
                        distance_by_id.setdefault(hit.photo_id, hit.distance)
                vector_lists.append(ids)
            vector_ids = vector_lists[0] if len(vector_lists) == 1 else _rrf_fuse(*vector_lists)
            vector_distances = [
                distance_by_id[photo_id]
                for photo_id in vector_ids
                if distance_by_id.get(photo_id) is not None
            ]
            note_ids: list[str] = []
            if note_count:
                # 같은 질의 임베딩 사용 — k는 컬렉션 크기로 클램프(과요청 경고 방지).
                note_hits = self.note_store.query(
                    embedding=embeddings[0],
                    k=min(pool_k, note_count),
                )
                vector_ids, note_ids = _fold_note_hits(vector_ids, vector_distances, note_hits)
            notes_bm25_ids = (
                self.notes_bm25.search(query, pool_k) if notes_bm25_count else []
            )
            candidate_ids = _rrf_fuse(
                ...
                note_ids,
                notes_bm25_ids,
                weights=(1.0, 1.0, _BROAD_LEXICAL_WEIGHT, 1.0, 1.0),
            )
            passed_all = self.db.filter_photo_ids(candidate_ids, filters)
            passed_ids = passed_all[:k]
            if len(passed_ids) >= k or store_exhausted:
                break
...
def _rrf_fuse(*ranked_lists: list[str], weights: tuple[float, ...] | None = None) -> list[str]:
    ...
    for ranked, weight in zip(ranked_lists, weights, strict=True):
        for rank, photo_id in enumerate(ranked, start=1):
            scores[photo_id] = scores.get(photo_id, 0.0) + weight / (_RRF_K + rank)
