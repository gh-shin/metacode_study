...
def write_variant_matrix(rows_by_variant: dict[str, list[GoldenRow]], report_path: Path) -> None:
    """변형별 GoldenRow를 PASS/FAIL/보류 격자로 쓴다."""
    variants = list(rows_by_variant)
    first_rows = next(iter(rows_by_variant.values()), [])
    rows_by_id = {
        variant: {row.id: row for row in rows} for variant, rows in rows_by_variant.items()
    }
    ids = [row.id for row in first_rows]
    for rows in rows_by_variant.values():
        for row in rows:
            if row.id not in ids:
                ids.append(row.id)

    def metric_row(verdict: str) -> str:
        counts = [
            str(sum(1 for row in rows if row.verdict == verdict))
            for rows in rows_by_variant.values()
        ]
        return f"| {verdict} | " + " | ".join(counts) + " |"
    ...
    lines = [
        ...
        metric_row("보류"),
        "",
        "| ID | Question | " + " | ".join(variants) + " |",
        "|---|---|" + "|".join("---" for _ in variants) + "|",
    ]
    for question_id in ids:
        question = next(
            (
                rows_by_id[variant][question_id].question
                for variant in variants
                if question_id in rows_by_id[variant]
            ),
            "",
        )
        verdicts = [
            rows_by_id[variant].get(question_id).verdict
            if question_id in rows_by_id[variant]
            else "-"
            for variant in variants
        ]
        lines.append(f"| {question_id} | {question} | " + " | ".join(verdicts) + " |")
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
