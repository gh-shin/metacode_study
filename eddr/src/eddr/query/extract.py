...
EXTRACT_FORMAT: dict = {
    ...
    "properties": {
        ...
        "answer_type": {"type": "string", "enum": ["fact", "photo_list"]},
    },
    "required": [
        "keywords_en",
        "keywords_ko",
...
class ExtractedQuery:
    ...
    Attributes:
        ...
        answer_type: fact면 여행 날짜·기간 같은 사실 질의, photo_list면 사진 목록 질의.
    """
...
def _system_prompt(today: date) -> str:
    ...
    return (
        ...
        "표시 전용이며 검색에는 쓰지 않는다.\n"
        "- answer_type: 날짜·기간·여행 시점을 묻는 질의는 fact, "
        "사진 내용을 찾는 질의는 photo_list.\n"
...
def _parse_content(content: str) -> ExtractedQuery:
    ...
    return ExtractedQuery(
        ...
        answer_type=_answer_type(data),
    )


_HOME_COUNTRY = "대한민국"
_ANSWER_TYPES = {"fact", "photo_list"}


def _answer_type(data: dict) -> str:
    """answer_type은 신규 필드라 누락·오분류 문자열은 사진 목록 질의로 강등한다."""
    value = data.get("answer_type", "photo_list")
    return value if isinstance(value, str) and value in _ANSWER_TYPES else "photo_list"
