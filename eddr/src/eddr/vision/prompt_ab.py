...
def run_prompt_ab(
    ...
    각 사진마다 ``P3_HYBRID_PROMPT_NAME``과 ``P3_HYBRID_V2_PROMPT_NAME`` 두 프롬프트를
    순서대로 호출한다. 두 캡션 모두 성공해야 processed로 집계되며, 어느 하나라도
    예외가 발생하면 failed로 집계된다.
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for photo in db.pending_vision_photos_stratified(limit=limit):
